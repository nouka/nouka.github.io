...
const toggleFavorite = (e) => {
  ...
  if (dbdata.favorites && dbdata.favorites[messageId]) {
    ...
    // favorites に該当のメッセージをお気に入りとして追加
...
const createMessageDiv = (messageId, message) => {
  ...
  if (dbdata.favorites && dbdata.favorites[messageId]) {
    mfl.find('i').removeClass('fa-star-o').addClass('fa-star');
  } else {
    mfl.find('i').removeClass('fa-star').addClass('fa-star-o');
  }
...
const loadChatView = () => {
  ...
  favoritesRef.on('child_removed', (favSnapshot) => {
    ...
    $(`#message-id-${messageId}`).find('i').removeClass('fa-star').addClass('fa-star-o');
  });
  ...
  favoritesRef.on('child_added', (favSnapshot) => {
    ...
    $(`#message-id-${messageId}`).find('i').removeClass('fa-star-o').addClass('fa-star');
  });
};
